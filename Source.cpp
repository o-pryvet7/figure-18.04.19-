...
the_end:
	...
	figure **F; int n;
	figure *pF;
	cout << " Input count of figures: "; cin >> n;
	...
	for (int i = 0; i < n; i++) {
		choise = "" ;
		cout << " Input type : " << endl; cin.get(); cin.getline(choise, (sizeof(choise) / sizeof(choise[0])), '\n');
		cout << choise;

		pF = new figure;

		if (choise == "cir" || "c" || "circle" || "C" || "Cir" || "Circle" || "круг" || "Круг" || "коло" || "Коло") {pF = new circle;}
		else
		if (choise == "sqr" || "s" || "square" || "S" || "Sqr" || "Square" || "квадрат" || "Квадрат") { pF = new square; }
